...
- `reg-installation-context` — The shared understanding of where Codex is installed, where bundled assets live, and where helper binaries can be found.
...
- `reg-input-queues-and-mailboxes` — The pending-input and inter-agent message queues that feed upcoming turns and background deliveries.
...
- `reg-connection-shutdown-state` — The per-connection shutdown gate and cleanup progress that stop new work, let safe work finish, and cancel leftovers if needed.
...
- `reg-agent-spawn-reservations` — The temporary reservation state that holds agent path/name/count slots during child-agent creation so partial spawns clean up correctly and limits remain consistent.
- `reg-cancellation-and-readiness-flags` — The shared async flags, cancellation tokens, and readiness signals used across subsystems to coordinate startup, long-running tasks, and teardown.
- `reg-install-fingerprint-state` — The managed-install version and file-fingerprint state used to decide whether bundled helpers or daemon binaries need refresh, replacement, or update actions.
- `reg-turn-task-registry` — The registry of background tasks driving active turns, including spawned task handles, lifecycle tracking, and completion coordination per thread.
