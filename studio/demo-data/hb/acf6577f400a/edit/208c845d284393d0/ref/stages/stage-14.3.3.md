...
`config` · `tool setup / schema creation`

This file exists so the system can tell the model, in a precise and machine-readable way, how to use the special tool that searches for other tools. The real problem it solves is discoverability: not every tool is handed to the model up front, so there needs to be a built-in way to ask, "What tools are available for this job?"

The main function builds a ToolSpec, which is the formal recipe for a tool. It defines two inputs: a required search text (`query`) and an optional maximum result count (`limit`). It also writes a human-facing description that explains what the tool does and when to use it.

An important detail is how it lists tool sources. The function receives a list of possible sources, such as external systems or catalogs, and turns that into a clean bullet list for the description. If the same source appears more than once, it keeps only one entry. If one copy has a description and another does not, it preserves the useful description instead of losing it. If there are no sources, it says so plainly.

The test focuses on that behavior: it checks that duplicate sources are collapsed correctly and that the final tool specification matches the expected text and parameter schema.
...
**Purpose**: Builds the formal specification for the tool-search feature so the rest of the system can expose it to the model. Someone uses this when assembling the list of available tools and wants the search tool to describe itself clearly, including where it can search.

**Data flow**: It takes in a list of searchable sources and a default result limit. From that, it creates a small input schema with `query` and `limit`, then scans the source list to remove duplicates and keep the best available description for each source. Finally, it combines those pieces into one ToolSpec value: a complete definition containing execution mode, explanatory text, and the parameter schema.

**Call relations**: This function is called by a higher-level constructor named `new`, which is putting together tool definitions for the system. In that larger flow, this function's job is to supply the finished spec for the special search tool so later stages can present it to the model and let the client execute it.
...
**Purpose**: Checks that the tool-search specification is rendered exactly as intended, especially when the same source appears more than once. It protects against confusing descriptions by proving the output stays clean and readable.

**Data flow**: The test feeds in a small sample list of sources, including a duplicate with and without a description, plus a default limit. It runs the builder function and compares the entire returned ToolSpec against the exact expected structure and text. The result is pass or fail: either the generated spec matches perfectly, or the test signals that something changed unexpectedly.

**Call relations**: This test exercises `create_tool_search_tool` directly rather than being part of the runtime flow. Its role in the bigger story is safety: when developers change the tool description logic, this test catches mistakes in deduplication, source listing, or parameter text before those mistakes reach users.
