...
- `reg-windows-sandbox-account-state` — The special local Windows sandbox accounts and readiness state that some restricted execution features depend on.
- `reg-request-serialization-state` — The shared scheduler state that keeps conflicting requests in safe order while allowing unrelated work to run in parallel.
- `reg-tool-parallelism-state` — The shared control state that decides which tool calls may run together, one-at-a-time, or must be cancelled/cleaned up.
...
- `reg-daily-sandbox-log-state` — The rolling per-day Windows sandbox command log state used to append sandbox activity into simple rotating local log files.
- `reg-helper-materialization-state` — The shared readiness state for packaged helper executables copied/materialized into local sandbox or shared bin locations for later execution.
